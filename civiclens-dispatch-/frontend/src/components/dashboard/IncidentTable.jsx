...
function SortableHeader({ field, label, currentField, direction, onSort, className }) {
  ...
  return (
    <th
      className={cn(
        'px-3 py-2.5 text-left text-caption text-text-muted uppercase tracking-widest',
    ...
    >
      <span className="inline-flex items-center gap-1">
        {label}
        {isActive && <Icon size={11} className="text-accent" />}
      </span>
    </th>
  )
}
    ...
    <div className="overflow-auto h-full rounded-lg border border-border">
      <table className="w-full border-collapse text-body">
        ...
        <thead className="sticky top-0 z-10 bg-surface border-b border-border">
          <tr>
            <th className="px-3 py-2.5 text-left text-caption text-text-muted uppercase tracking-widest w-16">ID</th>
            <SortableHeader field="incident_type" label="Type"     currentField={sortField} direction={sortDirection} onSort={onSort} />
            <th className="px-3 py-2.5 text-left text-caption text-text-muted uppercase tracking-widest">Description</th>
            <th className="px-3 py-2.5 text-left text-caption text-text-muted uppercase tracking-widest">Location</th>
            <SortableHeader field="severity"    label="Severity"  currentField={sortField} direction={sortDirection} onSort={onSort} className="w-24" />
            <SortableHeader field="risk_score"  label="Risk"      currentField={sortField} direction={sortDirection} onSort={onSort} className="w-28" />
